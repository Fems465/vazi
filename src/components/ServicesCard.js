...
const ServicesCard = ({ title, description, linkText, linkHref, iconSrc }) => {
  return (
    <div className=" flex flex-col border border-[#ede6d4] rounded-xl gradient-card2 text-left w-[22rem] h-[25rem] pl-7 pr-12 py-6">
      ...
      <div className="mt-6">
        <p className="text-sm lg:text-base lg:leading-5 text-[#6F898C]">
          {description}
        </p>
      </div>
      <div className=" mt-auto">
        <Link
          href={linkHref || "/"}
          className=" text-left border-[#e9ca871f] bg-gradient-to-r from-[rgba(254,178,1,0.5)] to-[#EDECE9] hover:from-[#0037402d] hover:to-[#5bcedf5b] px-4 pr-[13.5rem] py-1.5 rounded-md text-xs text-white font-base"
        >
          {linkText || "More"}
        </Link>
      </div>
